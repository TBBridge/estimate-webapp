...
export default function AdminEstimatesPage() {
  ...
  // フィルタを API クエリパラメータとして渡す（空文字は除外）
          ...
          <thead>
            <tr className="border-b border-stone-200/80 dark:border-stone-700/80">
              {["admin.estimates.no","admin.estimates.agency","admin.estimates.customer",
                ...
                "admin.estimates.amount","admin.estimates.status","admin.estimates.createdAt"].map((k) => (
                <th key={k} className="px-4 py-3 text-left font-medium text-[var(--color-ink-muted)]">{l(k)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              <tr key={e.id} className="border-b border-stone-100 last:border-0 hover:bg-stone-50 dark:border-stone-800 dark:hover:bg-stone-800/40">
                ...
                <td className="px-4 py-3 text-[var(--color-ink-muted)]">{DELIVERY_TYPES.find((d) => d.value === e.deliveryType)?.labelJa}</td>
                <td className="px-4 py-3 text-[var(--color-ink-muted)]">{CONTRACT_TYPES.find((c) => c.value === e.contractType)?.labelJa}</td>
